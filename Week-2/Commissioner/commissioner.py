...
search_terms = ['goodell', 'nflcommish', 'RayRice' 'Ray Rice']
...
class StdOutListener(StreamListener):
    ...
    def on_data(self, data):
        if not os.path.isfile(os.path.join(os.getcwd(), 'comiss_coords.csv')):
            c = open('comiss_coords.csv', 'w')
            c.write('long, lat\n')
            c.close()

        if not os.path.isfile(os.path.join(os.getcwd(), 'comiss_tweets.csv')):
            c = open('comiss_tweets.csv', 'w')
            c.write('tweet, time\n')
            c.close()

        t = open('comiss_tweets.csv', 'a+')
        c = open('comiss_coords.csv', 'a+')
